...
const api = require('../../api/composite/');
...
class ReferenceIdRegister{
    ...
    static parse(request){
        ...
        for(const subRequest of request.compositeRequest){
            const { url, referenceId } = subRequest;
            // try to infer prototype from url hash (https://github.com/kev4ev/sf-ent#urlTypeInfo)
            if(url.indexOf('#') > -1){
                const version = url.split('services/data/')[1].split('/')[0];
                const { subrequests } = api(version);
                const name = url.split('#')[1].split('_')[0];
                const proto = Object.values(subrequests).filter(cls => {
                    return [ cls?.name, cls?.constructor?.name].includes(name);
                })[0];
                if(proto){
                    Object.setPrototypeOf(subRequest, proto);
                } 
            }
