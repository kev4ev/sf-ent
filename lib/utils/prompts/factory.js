const promptSymbols = require('./symbols');
 ...
 * @property {function(input<string>, this<Question> ): boolean | Promise<boolean>} [onCancel] event handler when user hits keyboard ESC; MUST be implemented for prompt to be cancelleable; else will cause process exit
...
module.exports = (...questions) => {
    ...
    return questions.map(question => {
        if(!question[factoried]){
            ...
            const { validate, onCancel } = question;
            // this overrides the standard cancel() method from enquirer's base Prompt class (prompt.js)
            // allows prompts implementing onCancel to gracefully handle user exits
            question.cancel = async function(name, cancelAction){
                await getChalk();
                log(chalk.bgYellow(`\nPrompt ${this.name} cancelled before completion`));
                const CANCEL_SYM = Symbol.keyFor(promptSymbols.cancel);
                if(onCancel && typeof onCancel === 'function'){
                    this.emit('submit', (await onCancel.call(this, this.name, CANCEL_SYM, this)));
                } else{
                    this.emit('cancel', CANCEL_SYM);
                }
            };
            if(validate){
