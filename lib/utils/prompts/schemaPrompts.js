...
async function getObjectPrompts(){
    ...
    const getPrompt = () => {
        return {
            ...
            limit: 5,
            required: true,
            footer: () => objects ? '' : chalk.yellow('Enter ! for schema assist (requires org connection)'),
...
async function getFieldPrompts(sobject, options={}, getterOverride={}){
        ...
        return Object.assign(
            {
                ...
                required: true,
                limit: 5,
                footer: () => fieldChoices ? '' : chalk.yellow('Enter ! for schema assist (requires org connection)'),
            },
    ...
    const prompts = factory(
        getterPrompt({
            result: async function(field){ // non-arrow to get context "this"
                if(field === ASSIST){
                    await setClosureVars(true);
                ...
                lastSelected = field;
                if(field !== DONE) allSelected.push(field);
            ...
            validate(value){
                if(this.type?.() === 'autocomplete'){
                    if(value === ASSIST || value === DONE) return true;
                    if(value?.length > 0){
