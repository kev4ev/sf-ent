...
const symbols = require('../../utils/prompts/symbols');

const CMD_DONE = Symbol('done'); // local symbol

/**
 * @typedef {Object.<string, string>} CommandArgs
 */
...
class Command extends Diviner{
    ...
    constructor(args={}){
        ...
        function doneAppender(){
            ...
            // make bound done mock a Command with interactive meta, so library api includes 'done' method rather than '<done>'
            boundDone.interactiveMeta = () => { 
                return { name: Symbol.keyFor(symbols.done), hint: `close ${this.name} command` };
            };
            
            return Object.assign(own, { [CMD_DONE.description] : boundDone });
        }
    ...
    /**
     * Subclasses MAY override; defaults to false
     * @returns {boolean} 
     */
    ...
    async executeInteractive(maxRecursion = -1, iteration = 0){
        ...
        const { subCmd } = await prompt(this.promptFactory(
            {
                name: 'subCmd',
        ...
        if(subCmd === CMD_DONE.description){
