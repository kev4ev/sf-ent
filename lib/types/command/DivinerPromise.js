    ...
    extend(subDivinerHash){
        Object.entries(subDivinerHash).forEach(subDiviner => {
            if(subDiviner){
                ...
                this[key] = (...args) => {
                    // call relayed fn; if result is not Diviner, break relay chain and return the root promise
                    const instance = fn(...args);
                    if(!instance || !(instance instanceof Diviner)){
                        if(this.diviner.parent){
                            // TODO does this work in both styles? index.test.js
                            // return this.diviner.parent.getSubDiviners();
                        }
