 ...
 * @property {boolean | string | undefined} disabled
 */

/**
 * @typedef {object} SnippetField
 * @property {string} name
 * @property {string} [message]
 * @property {string} [initial]
 * @property {function(value<string>, state<object>, item<object>, index<number>): boolean} [validate]
 */
...
class Diviner extends EventEmitter{
        ...
        if(this.parent){
            ...
            switch (evt) {
                ...
                case 'done':
                    ...
                    // lastly, emit a structured clone (i.e. read) copy of payload, if possible
                    const clone = (()=>{
                        try{
     ...
     * @yields {Question | Array<Question>} GeneratorFunction that returns one or more Question objects 
     * when info is needed from the user. Calls to next() yield a new array until all info is gathered.
