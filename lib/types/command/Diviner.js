    ...
    async #dispatchLifecycleEvent(evt, payload){
        ...
        if(this.parent){
            ...
            switch (evt) {
                ...
                case 'done':
                    // first, fire payload to all NON-PARENT lifecycle handlers; this is done so that
                    // parent receives the final version of payload, after any transformations
                    try{
                        this.emit('done', payload);
                    } catch(err){
                        this[captureRejectionSymbol](err, evt);
                    }
                    // fill queue and invoke handler for each unfilled item, 
