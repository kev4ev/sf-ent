 ...
 * passed to their constructors and fire a payload to listeners after processing. Due to the 
 * patterns used in this library, SUBCLASSES of Diviner should almost always be instantiate by a wrapper 
 * function rather than directly (see DivinerPromise.divine()).
 * @class
 */
class Diviner {
    /**
    ...
    async run(){
        ...
        if(this.parent) await this.parent.handleSubDivinerEvent('called', this);
        // prerun lifecycle
        await this.preRun();
        ...
        try{
            ...
            // if parent, call 'done' handler
            if(this.parent) await this.parent.handleSubDivinerEvent('done', result);
            // resolve own DivinerPromise
            this.runResolver(result);
        } catch(err){
            // if parent, call 'error' handler
            if(this.parent) await this.parent.handleSubDivinerEvent('error', err);
            // reject own DivinerPromise
            this.runRejector(err);
        }
