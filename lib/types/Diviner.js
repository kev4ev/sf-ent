...
class Diviner {
    ...
    constructor(args={}){
        ...
        this.runRejector = (err) => {
            this.log(this.chalk.bgRed(err.message));
            ...
            _handleLifecycleEvent: {
                value: async (evt, payload) => {
                    ...
                    if(this.parent){
                        ...
                        await Promise.resolve(); // to keep "ticks" consistent
                    }
                }
            },
            _relay: {
    ...
    async run(){
        ...
        if(typeof readyStatus === 'string'){
            this.log(this.chalk.bgRed(`Unable to execute due to following error: ${readyStatus}`));
