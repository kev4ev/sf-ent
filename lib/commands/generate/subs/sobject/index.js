...
class SObject extends Command{
    ...
    getSubDiviners(){
        ...
        return {
            ...
            // only practically relevant for interactive use, though accessible via lib
            'READ (by external id)': wrap(subs.SObjectGET.initExt).bind(this), 
            UPDATE: wrap(subs.SObjectPATCH.init).bind(this),
            DELETE: wrap(subs.SObjectDELETE.init).bind(this),
            // only practically relevant for interactive use, though accessible via lib
            'DELETE (by external id)': wrap(subs.SObjectDELETE.initExt).bind(this)
        };
    }
