...
class SObjectGET extends SObjectSubrequest{
    ...
    async *getPrompts(){
        ...
        const ALL_SYM = Symbol.keyFor(this.promptSymbols.allFields),
            DONE_SYM = Symbol.keyFor(this.promptSymbols.done),
            ASSIST_SYM = Symbol.keyFor(this.promptSymbols.assist);

        let latest;
        const queryFieldsPrompt = await this.getFieldPrompts(
            this.sobject, 
            {
