...
const { getRegistry  } = require('../../utils/request/referenceRegistry');
...
class Generate extends Command{
    ...
    async handleSubDivinerEvent(evt, payload, diviner){
        ...
        if(evt === 'called'){
            switch(divinerName){
                case 'preview':
                    diviner.generateState = request(this.#subRequestBodies, this.allOrNone, this.collateSubrequests);
                    break;
                case 'modify':
        ...
        if(evt === 'done'){
            switch (divinerName){
                ...
                case 'sobject':
                    ...
                    this.#subRequestBodies.push(...(Array.isArray(payload) ? payload : [ payload ]));
                    break;
    ...
    async #finalize(){
        const result = request(this.#subRequestBodies, this.allOrNone, this.collateSubrequests),
