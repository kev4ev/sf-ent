...
class Load extends Command{
    ...
    async executeInteractive(){
        // load the request and make callout
        ...
        if(resp.status === 200){
            // handle teardown
            if(this.teardown){
                // TODO use library classes
                const teardown = { allOrNone: true, collateSubrequests: false, compositeRequest: [ ] };
                resp.data.compositeResponse.forEach((res, index) => {
                    ...
                    if(httpStatusCode === 201 && httpHeaders?.Location){
                        ...
                        teardown.compositeRequest.push({
                            method: 'DELETE',
                            url: `${base.replace(instanceUrl, '').replace(/^services/, '/services')}/sobjects/${sobject}/${id}/`,
                            referenceId: `SObjectDELETE_${sobject}_${id}`
                        });
                    }
                });
                // write the file
