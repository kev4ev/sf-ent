...
const { cancel, done } = require('../../utils/prompts/symbols');
...
class Modify extends Command{
    ...
    async *getPrompts(){
        ...
        if(!this.currentOnly){
            const WRITE_IT = 'Yes, write to file.',
                choices = [ WRITE_IT, Symbol.keyFor(cancel) ];
            yield this.promptFactory(
                {
                    name: 'writeToFile',
                    message: `Ready to update file ${this.in}?`,
                    type: 'autocomplete',
                    choices
                }
            );
            if(this.writeToFile === WRITE_IT){
